...
describe("milestone rewards", () => {
  ...
  it("unlocks permanent rewards at the first three milestone thresholds", async () => {
  ...
  it("never returns the same reward twice", async () => {
    await unlockRewardsForMilestoneCount(3);
    expect(await unlockRewardsForMilestoneCount(3)).toEqual([]);
  });
