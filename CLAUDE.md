...
- **2026-07-17 GROUND-UP SPRINT (uncommitted on main):** full strategy review
  ...
  telemetry + entitlement re-sync, "Today's Progress" widget/App Intents
  (`targets/widget/`, @bacons/apple-targets, App Group
  `group.com.resolutioncompanion.app`), hook-portfolio reminders, Month in
  Monthly Progress recap + share, Insights panel, 2-shield economy, MI coach + real
  ...
  Health auto-completion + Siri. New deps: @bacons/apple-targets,
  ...
  - **Regression QA:** `qa/maestro-regression.yaml` (fresh install → paywall)
    ...
    Regression also caught+fixed a pending-widget-completion race: the AppState
    'active' listener could consume completions before the store loaded (guarded
