...
export const planTuneUpRequestSchema = z.object({
...
export const planTuneUpResponseSchema = z
  .object({
    slot: z.number().int().min(0).max(4),
    changes: z.object({
...
export type PlanTuneUpRequest = z.infer<typeof planTuneUpRequestSchema>;
export type PlanTuneUpResponse = z.infer<typeof planTuneUpResponseSchema>;

export function parsePlanTuneUpResponse(
  value: unknown,
  request: PlanTuneUpRequest,
): PlanTuneUpResponse {
  const parsed = planTuneUpResponseSchema.parse(value);
